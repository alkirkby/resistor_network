...
from rnpy.functions.assignfaults_new import (
    get_Nf,
    get_alpha,
    add_random_fault_sticks_to_arrays,
)
...
def parse_arguments(arguments):
    ...
    argument_names = [
        ...
        [
            "probability_z",
            "pz",
            "probability of a fault in z direction",
            1,
            float,
        ],
        [
            "direction",
            "d",
            "direction of flow through fractures, y (parallel to offset) or z (perpendicular)",
            1,
            str,
        ],
...
@delayed
def setup_and_solve_fault_sticks(
    Nf,
    fault_lengths_m,
    fault_widths,
    hydraulic_apertures,
...
if __name__ == "__main__":
    ...
    max_y_fault_length = (
        inputs["maximum_y_fault_length"]
        if "maximum_y_fault_length" in inputs.keys()
        else None
    )
    ...
    lvals_center, lvals_range, fw, aph, resistivity = read_fault_params(
        os.path.join(wd, "fault_k_aperture_rf%s_%s.npy" % (rfluid, direction)),
        None,
    )
    ...
    if len(np.unique(lvals_center)) < len(lvals_center):
        ...
        fw_mean = np.array(
            [np.mean(fw[lvals_center == ll]) for ll in lvals_center_unique]
        )
    else:
        lvals_center_unique, lvals_range_unique, fw_mean = (
            lvals_center,
            lvals_range,
            fw,
        )
    ...
    Nf = get_Nf(a, alpha, R, lvals_range_unique)
    ...
    if lmax is None:
        lmax, idx1 = (
            np.amax(lvals_range_unique) + 1,
            len(lvals_range_unique) + 1,
        )
    else:
        idx1 = (
    ...
    with open(os.path.join(wd, output_fn), "w") as openfile:
        ...
        openfile.write(
            "# fault_lengths_center "
            + " ".join([str(val) for val in lvals_center_unique])
            + "\n"
        )
        openfile.write(
            "# fault_lengths_bin_ranges "
            + " ".join([str(val) for val in lvals_range_unique])
            + "\n"
        )
        openfile.write(
    ...
    np.savez_compressed(
        os.path.join(os.path.join(array_savedir, "arrays")),

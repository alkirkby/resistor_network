...
class Rock_volume:
    ...
    ***************Documentation last updated 21 June 2016, not finished*******************
    ...
    def __init__(self, **input_parameters):
        ...
        self.resistivity_matrix = 1000.0
        self.resistivity_fluid = 0.1
        self.matrix_current = True
        self.matrix_flow = True
        self.resistivity = None
        self.permeability_matrix = 1.0e-18
        self.permeability_gouge = 1.0e-14
        self.porosity_gouge = 0.3
        self.fluid_viscosity = (
            1.0e-3  # default is for freshwater at 20 degrees
        )
        ...
        self.gouge_fraction = (
            0.0  # fraction of the void space that is filled with gouge
        )
        self.gouge_area_fraction = (
            0.0  # fraction of the total area of the fault that has gouge
        )
        ...
        if type(self.cellsize) in [float, int]:
            ...
            if (type(self.fault_assignment) == "single_yz") and len(
                self.cellsize == 3
            ):
                if self.cellsize[1] != self.cellsize[2]:
                    print(
                        "y cellsize not equal to z cellsize, updating z cellsize"
                    )
    ...
    def _trim_arrays(self):
        if np.all(self.fault_array.shape[:3] > np.array(self.ncells) + 2):
            ...
            self.fault_array = rna.add_nulls(
                self.fault_array[buf:-buf, buf:-buf, buf:-buf]
            )
            self.fault_edges -= self.array_buffer
        if np.all(self.aperture.shape[:3] > np.array(self.ncells) + 2):
            buf = self.array_buffer
            self.aperture = rna.add_nulls(
                self.aperture[buf:-buf, buf:-buf, buf:-buf]
            )

        self.resistance = rna.add_nulls(
            self.resistance[buf:-buf, buf:-buf, buf:-buf]
        )
        self.resistivity = rna.add_nulls(
            self.resistivity[buf:-buf, buf:-buf, buf:-buf]
        )
        self.aperture_electric = rna.add_nulls(
            self.aperture_electric[buf:-buf, buf:-buf, buf:-buf]
        )

        self.hydraulic_resistance = rna.add_nulls(
            self.hydraulic_resistance[buf:-buf, buf:-buf, buf:-buf]
        )
        self.permeability = rna.add_nulls(
            self.permeability[buf:-buf, buf:-buf, buf:-buf]
        )
        self.aperture_hydraulic = rna.add_nulls(
            self.aperture_hydraulic[buf:-buf, buf:-buf, buf:-buf]
        )
    ...
    def build_faults(self, create_array=True):
        ...
        if self.fault_array is not None:
            if self.fault_array.shape != (
                nz + 2 + self.array_buffer * 2,
                ny + 2 + self.array_buffer * 2,
                nx + 2 + self.array_buffer * 2,
                3,
                3,
            ):
                print(
                    "Fault array does not conform to dimensions of network, creating a new array!"
                )
                self.fault_array = None
        ...
        if self.fault_array is None:
            if create_array:
                ...
                if self.fault_edges is not None:
                    self.fault_edges = (
                        np.array(self.fault_edges) + self.array_buffer
                    )
            ...
            if (self.fault_assignment == "list") or (
                self.fault_assignment[0] == "list"
            ):
                if self.fault_edges is not None:
            ...
            # option to specify a single fault in the centre of the yz plane
                ...
                iy0, iy1 = 1, ny + 1
                iz0, iz1 = 1, nz + 1
                self.fault_edges = np.array(
                    [
                        [
                            [[ix, iy0, iz0], [ix, iy1, iz0]],
            ...
            # option to specify a single fault in the centre of the xy plane
                ...
                self.fault_dict["fault_spacing"] = int(
                    self.fault_dict["fault_spacing"]
                )
                if nx > 1:
                    start = 2
                else:
                ...
                self.fault_edges = np.array(
                    [
                        ...
                        for ix in range(
                            start, nx + 2, self.fault_dict["fault_spacing"]
                        )
                    ]
                )
                addfaults = True
                ...
                self.fault_edges = np.array(
                    [
                        ...
                        for iy in range(
                            start, ny + 2, self.fault_dict["fault_spacing"]
                        )
                    ]
                )
                addfaults = True
                ...
                self.fault_edges = np.array(
                    [
                        ...
                        for iz in range(
                            start, nz + 2, self.fault_dict["fault_spacing"]
                        )
                    ]
                )
                addfaults = True
            ...
            # single fault in each of the xy, yz, and xz planes
                ...
                pxyz = np.array(self.pconnection) / float(
                    sum(self.pconnection)
                )
                # get fault edges
                fracturecoords = rnaf.get_fracture_coords(
                    lvals,
                    networksize,
                    pxyz,
                    return_Nf=False,
                    a=self.fault_dict["a"],
                    alpha=self.fault_dict["alpha"],
                )
                self.fault_edges = rnaf.coords2indices(
                    fracturecoords, networksize, [nx, ny, nz]
                )
                addfaults = True
            else:
            ...
            if type(self.fault_dict["fault_separation"]) in [float, int]:
                self.fault_dict["fault_separation"] *= np.ones(
                    len(self.fault_edges)
                )
    ...
    def build_aperture(self):
        ...
        if self.fault_assignment == "single_yz":
            ...
            planelist = np.array(
                [
                    rnfa.get_plane(self.fault_edges[i])
                    for i in range(len(self.fault_edges))
                ]
            )
        ...
        if self.fault_assignment == "none":
            ...
            if self.build_arrays:
                (
                    ...
                    apc,
                    self.aperture,
                    self.aperture_hydraulic,
                    self.aperture_electric,
                    self.fault_dict["fault_surfaces"],
                    self.overlap_volume,
                ) = rnaf.assign_fault_aperture(
                    self.fault_edges,
                ...
                for i in range(3):
                    ap_min = np.max(
                        [
                            (self.permeability_matrix[i] * 12) ** 0.5,
                            (self.fault_dict["minimum_aperture"]),
                        ]
                    )
                    self.aperture_hydraulic[:, :, :, i][
                        self.aperture_hydraulic[:, :, :, i] < ap_min
                    ] = ap_min
    ...
    def _get_faulted_apertures(self):
        ...
        for j in [0, 1, 2]:
            ...
            idxs.remove(j)
            apvals = np.nanmax(
                [self.aperture[:, :, :, i, j] for i in idxs], axis=0
            )
            fmask = np.nanmax(
                [self.fault_array[:, :, :, i, j] for i in idxs], axis=0
            )
    ...
    def _get_contact_area(self):
        ...
        for j in [0, 1, 2]:
            ...
            # set a minimum value
            min_ap = max(
                self.fault_dict["minimum_aperture"] * (1 + 1e-6), 1e-49
            )
    ...
    def get_xyz_apertures(self):
        ...
        apx[:-1] = aperture[
            1:-1, 1:, 1:, 2, 0
        ]  # z connectors opening in x direction (yz)
        apx[:, :-1] = aperture[
            1:, 1:-1, 1:, 1, 0
        ]  # y connectors opening in x direction (yz)
        ...
        apy[:-1] = aperture[
            1:-1, 1:, 1:, 2, 1
        ]  # z connectors opening in y direction
        apy[:, :, :-1] = aperture[
            1:, 1:, 1:-1, 0, 1
        ]  # x connectors opening in y direction
        # fill missing aperture in corner
        try:
    ...
    def update_cellsize(self):
        if (
            self.fault_assignment
            in [
                pre + suf
                for pre in ["single_", "multiple_"]
                for suf in ["xy", "yz", "xz"]
            ]
        ) or (min(self.ncells) == 0):
            if min(self.ncells) == 0:
                i = np.where(np.array(self.ncells) == 0)[0][0]
            else:
                i = ["yz", "xz", "xy"].index(
                    self.fault_assignment.split("_")[-1]
                )
            apih = self.aperture_hydraulic[:, :, :, :, i][
                np.isfinite(self.aperture_hydraulic[:, :, :, :, i])
            ]
            apie = self.aperture_electric[:, :, :, :, i][
                np.isfinite(self.aperture_electric[:, :, :, :, i])
            ]
            ...
            for api in [apih, apie]:
                if len(api) > 0:
                    ...
                    if self.cellsize[i] < apmax:
                        rounding = (
                            -int(np.ceil(np.log10(self.cellsize[i]))) + 2
                        )
                        # need to use ceil function so it always rounds up
                        self.cellsize[i] = np.ceil(
                            apmax * 10.0**rounding
                        ) * 10.0 ** (-rounding)
    ...
    def initialise_electrical_resistance(self):
        ...
        self.resistance, self.resistivity, self.aperture_electric = (
            rnap.get_electrical_resistance(
                self.aperture_electric,
    ...
    def initialise_permeability(self):
        ...
        self.hydraulic_resistivity = rnap.get_hydraulic_resistivity(
            self.hydraulic_resistance, self.cellsize
        )
    ...
    def add_fault_gouge(self):
        ...
        if gouge_vol >= vol:
            ...
            if gouge_vol / vol < 0.005:
                self.gouge_fraction = 0.0
                self.gouge_area_fraction = 0.0
                ...
                while count < 100:
                    ...
                    count += 1
                self.gouge_area_fraction = (
                    (1.0 - self.contact_area[0])
                    * (
                        np.sum(filt[1:-1, 1:, 1, 2])
                        + np.sum(filt[1:, 1:-1, 1, 1])
                    )
                    / +(filt[1:-1, 1:, 1, 2].size + filt[1:, 1:-1, 1, 1].size)
                )
        ...
        for i in range(3):
            hydres_gouge = (
                ...
                * self.cellsize[i]
                / (
                    self.aperture_electric[:, :, :, i, 0]
                    * self.permeability_gouge
                    + (
                        self.cellsize[i]
                        - self.aperture_electric[:, :, :, i, 0]
                    )
        ...
        for pname in list(property_arrays.keys()):
            ...
            for dname, nn in [["x", nx], ["y", ny], ["z", nz]]:
                if dname in self.solve_direction:
                    if nn == 0:
                        self.solve_direction = self.solve_direction.strip(
                            dname
                        )
                        print(
                            "not solving {} as there are no resistors in this direction".format(
                                dname
                            )
                        )

            if "x" in self.solve_direction:
                prop = 1.0 * property_arrays[pname].transpose(2, 1, 0, 3)
                prop = prop[:, :, :, ::-1]
                ...
                oa[1:, 1:, :, 0, 0] = (
                    c[-nfz:].reshape(nz + 2, ny + 1, nx + 1).transpose(2, 1, 0)
                )
                oa[1:, 1:-1, 1:, 0, 1] = (
                    c[nfx:-nfz].reshape(nz + 1, ny, nx + 1).transpose(2, 1, 0)
                )
                oa[1:-1, 1:, 1:, 0, 2] = (
                    c[:nfx].reshape(nz + 1, ny + 1, nx).transpose(2, 1, 0)
                )

            if "y" in self.solve_direction:
                # transpose array as y direction is now locally the z direction
                prop = 1.0 * property_arrays[pname].transpose(1, 0, 2, 3)
                ...
                nfx, nfy, nfz = rnmb.get_nfree([nx, ny, nz])
                oa[1:, 1:, 1:-1, 1, 0] = (
                    c[:nfx].reshape(nz + 1, ny + 1, nx).transpose(1, 0, 2)
                )
                oa[1:, :, 1:, 1, 1] = (
                    c[-nfz:].reshape(nz + 2, ny + 1, nx + 1).transpose(1, 0, 2)
                )
                oa[1:-1, 1:, 1:, 1, 2] = (
                    c[nfx:-nfz].reshape(nz + 1, ny, nx + 1).transpose(1, 0, 2)
                )

            if "z" in self.solve_direction:
                prop = 1.0 * property_arrays[pname]
                matrix, b = rnmb.build_matrix3d(prop)
                c = rnms.solve_matrix(matrix, b)
                nz, ny, nx = np.array(np.shape(prop))[:-1] - 2
                nfx, nfy, nfz = rnmb.get_nfree([nx, ny, nz])
                oa[1:, 1:, 1:-1, 2, 0] = c[:nfx].reshape(nz + 1, ny + 1, nx)
                oa[1:, 1:-1, 1:, 2, 1] = c[nfx:-nfz].reshape(
                    nz + 1, ny, nx + 1
                )
            ...
            if "current" in pname:
                ...
                self.resistivity_bulk, self.resistance_bulk = (
                    rnap.get_bulk_resistivity(self.current, self.cellsize, 1.0)
                )

            if "fluid" in pname:
                self.flowrate = 1.0 * oa
                self.permeability_bulk, self.hydraulic_resistance_bulk = (
                    rnap.get_bulk_permeability(
                        self.flowrate, self.cellsize, self.fluid_viscosity, 1.0
                    )
                )

    def solve_resistor_network2(
        self,
        Vstart=None,
        Vsurf=0.0,
        Vbase=1.0,
        ...
        for pname in list(property_arrays.keys()):
            ...
            for sd in self.solve_direction:
                ...
                output_array[1:-1, 1:, 1:, i, 2] = (
                    (Vn[1:] - Vn[:-1]) * dx * dy / (R[1:-1, 1:, 1:, 2] * dz)
                )
                output_array[1:, 1:-1, 1:, i, 1] = (
                    (Vn[:, 1:] - Vn[:, :-1])
                    * dx
                    * dz
                    / (R[1:, 1:-1, 1:, 1] * dy)
                )
                output_array[1:, 1:, 1:-1, i, 0] = (
                    (Vn[:, :, 1:] - Vn[:, :, :-1])
                    * dy
                    * dz
                    / (R[1:, 1:, 1:-1, 0] * dx)
            ...
            if pname == "current":
                ...
                for ii in range(3):
                    self.resistivity_bulk[ii] = min(
                        self.resistivity_bulk[ii], self.resistivity_matrix[ii]
                    )
            elif pname == "fluid":
                self.pressure[:, :, :, i] = Vn
                self.flowrate = output_array * 1.0
                self.permeability_bulk, self.hydraulic_resistance_bulk = (
                    rnap.get_bulk_permeability(
                        self.flowrate,
                        self.cellsize,
                ...
                for ii in range(3):
                    self.permeability_bulk[ii] = max(
                        self.permeability_bulk[ii],
    ...
    def get_effective_apertures(self):
        ...
        for i in range(3):
            if "xyz"[i] in self.solve_direction:
                for odir in range(3):
                    if odir != i:
                        width = self.cellsize[odir] * (self.ncells[odir] + 1.0)
                        if "current" in self.solve_properties:
                            rhoeff = self.resistivity_bulk[i]
                            self.effective_electric_aperture[i, odir] = (
                                rnap.get_electric_aperture(
                                    width, rhoeff, rhof, rhom[i]
                                )
                            )

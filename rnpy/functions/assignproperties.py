...
def update_all_apertures(aperture_array, d):
    ...
    for i in range(3):
        ...
        for od in dpi:
            for k1, j1, i1 in np.array(
                np.where(aperture_array[:, :, :, i, od] > d[od])
            ).T:
                ncf = 3
                ...
                apedge = (apval - (ncf - 2) * d[od]) / 2.0

                # set the extra bits, first the internal points just set to minres
                ind = int(ncf / 2) - 1

                aperture_array, ind0, ind2 = update_apertures(
                    aperture_array, i, k1, j1, i1, ind, od, d, apedge
                )
...
def get_electrical_resistance(
    aperture_array, r_matrix, r_fluid, d, matrix_current=False
):
    """
    ...
    for i in range(3):
        ...
        for ii in range(2):
            ...
            area_fracture += (
                aperture_array[:, :, :, i, dpi[ii]] * d[dpi[1 - ii]]
            )
        # subtract the overlapping bit if there is any
        area_fracture -= (
            aperture_array[:, :, :, i, dpi[0]]
            * aperture_array[:, :, :, i, dpi[1]]
        )
        area_fracture[area_fracture < 0.0] = 0.0

        # subtract fracture area from matrix area and remove any negative matrix area
        area_matrix -= area_fracture
        area_matrix[area_matrix < 0.0] = 0.0
        ...
        if matrix_current:
            resistance_array[:, :, :, i] = np.amin(
                [
                    resistance_array[:, :, :, i],
                    d[i]
                    / (area_matrix / r_matrix[i] + area_fracture / r_fluid),
                ],
                axis=0,
            )
        else:
    ...
    for i in range(3):
        ...
        resistance_array[:, :, :, i][cond] = np.amin(
            [
                r_fluid * aperture_array[:, :, :, i, i][cond] / np.prod(dp)
                + r_matrix[i]
                * (d[i] - aperture_array[:, :, :, i, i][cond])
                / np.prod(dp),
        ...
        resistivity_array[:, :, :, i] = (
            resistance_array[:, :, :, i] * np.prod(dp) / d[i]
        )
...
def get_permeability(aperture_array, k_matrix, d):
    ...
    for i in range(3):
        permeability_array[:, :, :, i] = (
            aperture_array[:, :, :, i] ** 3 / 12.0
            + (ln[i] - aperture_array[:, :, :, i]) * k_matrix[i]
        ) / ln[i]
...
def get_hydraulic_resistance_old(aperture_array, k_matrix, d, mu=1e-3):
    ...
    for i in range(3):
        ...
        # subtract the overlapping bit if there is any
        area_fracture -= (
            aperture_array[:, :, :, i, dpi[ii]]
            * aperture_array[:, :, :, i, dpi[ii - 1]]
        )
        ...
        hresistance[:, :, :, i] = (
            ...
            * d[i]
            / (
                d[dpi[1]] * aperture_array[:, :, :, i, dpi[0]] ** 3 / 12.0
                + d[dpi[0]] * aperture_array[:, :, :, i, dpi[1]] ** 3 / 12.0
        ...
        permeability[:, :, :, i] = (
            mu
            * d[i]
            / (hresistance[:, :, :, i] * (area_fracture + area_matrix))
        )

    return hresistance, permeability


def get_hydraulic_resistance(
    aperture_array,
    hydraulic_aperture_array,
    k_matrix,
    d,
    mu=1e-3,
    matrix_flow=True,
    ...
    hydres = 12.0 * mu / hydraulic_aperture_array**2.0
    for i in range(3):
        hydres[:, :, :, i][hydres[:, :, :, i] > mu / k_matrix[i]] = (
            mu / k_matrix[i]
        )
    ...
    for i in range(3):
        ...
        for od in dpi:
            ...
            for k1, j1, i1 in np.array(
                np.where(hydraulic_aperture_array[:, :, :, i, od] > d[od])
            ).T:
                apval = hydraulic_aperture_array[k1, j1, i1, i, od]
                ...
                if od == 0:
                    ...
                    hydres[k1 : k1 + 2, ind0 - 1 : ind2 + 1, i1 : i1 + 2, i][
                        hydres[
                            k1 : k1 + 2,
                            ind0 - 1 : ind2 + 1,
                            i1 : i1 + 2,
                    ...
                    hydres[ind0 - 1 : ind2 + 1, j1 : j1 + 2, i1 : i1 + 2, i][
                        hydres[
                            ind0 - 1 : ind2 + 1,
                            j1 : j1 + 2,
                            i1 : i1 + 2,
            ...
            hydraulic_aperture_array[:, :, :, i, od][
                hydraulic_aperture_array[:, :, :, i, od] > d[od]
            ] = d[od]
    for i in range(3):
        # the two directions perpendicular to direction of flow, indices and values
        dpi = [dd for dd in range(3) if dd != i]
        ...
        hresistance[:, :, :, i] = np.amin(
            [hrnew, hresistance[:, :, :, i]], axis=0
        )
        # assign connectors in direction of opening (in the case where faults
        # are more than one cell width)
        cond = aperture_array[:, :, :, i, i] > 0
        hresistance[:, :, :, i][cond] = np.amin(
            [
                hydres[:, :, :, i, i][cond]
                * aperture_array[:, :, :, i, i][cond]
                / np.prod(dp)
                + mu
                * (d[i] - aperture_array[:, :, :, i, i][cond])
                / (np.prod(dp) * k_matrix[i]),
                hresistance[:, :, :, i][cond],
            ],
            axis=0,
        )
        ...
        permeability[:, :, :, i] = (
            mu * d[i] / (hresistance[:, :, :, i] * np.prod(dp))
        )
...
def get_hydraulic_resistivity(hresistance, cellsize):
    ...
    hresistivity = hresistance * 0.0

    for i in range(3):
        dpi = [cellsize[dd] for dd in range(3) if dd != i]
        hresistivity[:, :, :, i] = (
            hresistance[:, :, :, i] * np.prod(dpi) / cellsize[i]
        )
...
def effectiveres(b, rhoeff, rhof, rhom, width):
    ...
    """
    return 1.0 / rhoeff - (b / rhof + (width - b) / rhom) / width
...
def get_electric_aperture(width, rhoeff, rhof, rhom):
    ...
    if rhof > rhoeff:
        ...
        print(
            "can't calculate effective aperture, rhoeff must be finite and > 0"
        )
        return

    return so.newton(
        effectiveres, 0.0, args=(rhoeff, rhof, rhom, width), maxiter=100
    )
...
def effectivek(bh, keff, km, width):
    ...
    """
    return keff - (bh**3 / 12.0 - (width - bh) * km) / width
...
def get_hydraulic_aperture(width, keff, km):
    ...
    if keff <= km:
        print(
            "keff is %.3e which is < km (%.3e), setting effective aperture to 0.0"
            % (keff, km)
        )
        return 0.0
    if np.isinf(keff):
        print(
            "can't calculate effective aperture, keff must be finite, setting to 0.0"
        )
        return 0.0
    else:
        # to get a starting value for bh, approximate bh << width
        bhstart = (width * 12 * (keff - km)) ** (1.0 / 3)
        return so.newton(
            effectivek, bhstart, args=(keff, km, width), maxiter=100
        )
